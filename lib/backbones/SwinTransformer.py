...
def SwinB(pretrained=True):
    ...
    if pretrained is True:
        model.load_state_dict(torch.load('/home/sln/pretrained/swin_base_patch4_window12_384_22kto1k.pth', map_location='cpu')['model'], strict=False)
...
# Instantiate SwinB model
# Parameters for SwinB are: embed_dim=128, depths=[2, 2, 18, 2], num_heads=[4, 8, 16, 32], window_size=12
model = SwinTransformer(embed_dim=128, depths=[2, 2, 18, 2], num_heads=[4, 8, 16, 32], window_size=12)

# Calculate the number of parameters
total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
print(f"Total parameters in SwinB: {total_params / 1e6:.3f} M")
