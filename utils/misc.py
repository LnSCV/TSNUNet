...
def parse_args():
    ...
    #     cuda_visible_devices = [int(i) for i in os.environ["CUDA_VISIBLE_DEVICES"].split(',')] #CUDA_VISIBLE_DEVICES = 0, 1
    # if "LOCAL_RANK" in os.environ.keys():
    #     local_rank = int(os.environ["LOCAL_RANK"])
    #     print("os.environ[LOCAL_RANK]",local_rank)
    #
    # if local_rank == -1:
    #     device_num = 1
    #     device = 1
    ...
    #     print("device_num = len(cuda_visible_devices)",device_num)
    args.device = 1 #device
    args.device_num = 1
    args.local_rank = -1
    print('device_num',args.device_num)
    # args.device_num = device_num #1
    # # print("args.device_num",args.device_num)
    # args.local_rank = local_rank #-1
    # print("####args.local_rank = local_rank####",args.local_rank)
...
def to_cuda_device(sample,device):
    for key in sample.keys():
        if type(sample[key]) == torch.Tensor:
            # print("to_cuda(sample,device)",device)
            sample[key] = sample[key].cuda(device =device)
    return sample
def to_cuda(sample):
    for key in sample.keys():
        if type(sample[key]) == torch.Tensor:
