...
class RGB_Dataset(Dataset):
    def __init__(self, root, sets, tfs):
        ...
        for set in sets:
            image_root, gt_root = os.path.join(root, set, 'images'), os.path.join(root, set, 'gts')

            images = [os.path.join(image_root, f) for f in os.listdir(image_root) if f.lower().endswith(('.jpg', '.png','.bmp'))]
            images = sort(images)
